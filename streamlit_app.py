...
# マスタ設定 ページ
    ...
    st.title("⚙️ マスタデータ設定")
    st.write("ここにマスタデータ設定のコンテンツを配置します。")
    # ここに2_Master_Data_Settings.pyの内容をコピー＆ペーストしてください
    # 例: st.text_input("項目名", "初期値")
    # st.button("設定を保存")
    
    # 仮のマスターデータ表示（実際のロジックに置き換えてください）
    st.subheader("現在のマスタデータ（仮）")
    st.dataframe(pd.DataFrame({"ID": [1, 2], "項目": ["データA", "データB"]}))
